...
| --- | --- | --- | --- | --- | --- | --- |
| Done | 2026-07-17 | Robot Data / Simulation Data / General Data | Representative supervision datasets batch | User-provided 50-entry LaTeX table plus `DATASET_PAPER_DATA_LINKS.md` | Dataset headers contain only Project/Paper links; innermost rows contain only concrete dataset/download/annotation/access links | Added 40 new named dataset sections with verified data targets: 6 Robot Data, 4 Simulation Data / Benchmark, and 30 General Data. Reused the existing Ego4D and DexGraspNet 2.0 sections. Added BibTeX citations from official arXiv exports, OpenReview, CVF, and project citation blocks; corrected the original paper targets for EgoTaskQA, Objects365, HiREST, ScanQA, and InfLevel. Kept demos, environments, and licenses as `TBD` where the source links do not establish row-compatible values. Project homepages are no longer reused as data links. Removed RoVid-X, RobAVA, RoboFail, WAP, LLaVA-OneVision-SI, LLaRP / Language Rearrangement, RoboAfford++, and Moment-10M from the website table because no concrete official data target was verified; their research notes remain in `DATASET_PAPER_DATA_LINKS.md` for a future release check. |
