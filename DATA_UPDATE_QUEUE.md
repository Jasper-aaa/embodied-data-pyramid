...
| --- | --- | --- | --- | --- | --- | --- |
| Done | 2026-07-20 | UMI Data | Community-link cleanup and Daimon-Infinity labels | https://modelscope.cn/datasets/daimonrobotics/Daimon-Infinity | UMI dataset headers retain only dataset-specific project resources; Daimon-Infinity uses separate section and row labels | Removed the shared `Community` link from all UMI dataset headers and deleted the now-unused community URL constant; renamed the Daimon-Infinity task row from `Daimon-Infinity dataset` to `Daimon-Infinity`; labeled its section link as `Project` and its row-level data link as `LeRobot`. |
| Done | 2026-07-20 | UMI Data | Daimon-Infinity dataset link | https://modelscope.cn/datasets/daimonrobotics/Daimon-Infinity | Official ModelScope dataset page used for both the section-level and row-level dataset links | Replaced the incorrect UMI community header link and populated the previously empty task-row Data Links cell so every Daimon-Infinity dataset entry points to the official gated repository. |
| Done | 2026-07-18 | Ego Data / General Data | Data-link and resource-row audit | Official dataset portals, repositories, and direct archives | Dataset headers contain only Project/Paper navigation; concrete data/access resources remain in task rows; resource observations describe the linked artifact rather than the whole dataset | Removed the duplicated first Dataset link from every Ego dataset header; added resource-aware Ego observation inference and explicit THU-READ RGB/depth/annotation/split labels; replaced the broken EgoHands API endpoint with its dataset record; tightened General Data links for RH20T-P, SuctionNet, CLEVRER, InfLevel, TextOCR, PACO-LVIS, RefCOCO, Charades-STA, and SQA3D to download sections, files, annotations, or official access records. Unknown Ego and General fields remain blank. |
| Done | 2026-07-17 | General Data | Representative supervision datasets batch | User-provided 50-entry LaTeX table plus `DATASET_PAPER_DATA_LINKS.md` | All new sections are indexed under General Data; dataset headers contain only Project/Paper links and innermost rows contain only concrete dataset/download/annotation/access links | Added 40 new named dataset sections with verified data targets to General Data only. Robot Data and Simulation Data retain their pre-existing sections and are not extended by this batch. Reused the existing Ego4D and DexGraspNet 2.0 sections. Added BibTeX citations from official arXiv exports, OpenReview, CVF, and project citation blocks; corrected the original paper targets for EgoTaskQA, Objects365, HiREST, ScanQA, and InfLevel. Unknown demos, environments, licenses, and other fallback items are left blank in Ego Data and General Data instead of displaying `TBD` or `To be done`. Project homepages are no longer reused as data links. Removed RoVid-X, RobAVA, RoboFail, WAP, LLaVA-OneVision-SI, LLaRP / Language Rearrangement, RoboAfford++, and Moment-10M from the website table because no concrete official data target was verified; their research notes remain in `DATASET_PAPER_DATA_LINKS.md` for a future release check. |
| Done | 2026-07-17 | Robot Data / Simulation Data | Verified `# Demos` backfill | Official project pages, papers, and repository metadata | Dataset titles use full-release totals; task rows use official per-task counts where available | Updated AIST-Bimanual's title to the current official CSV total of 12,025 episodes across 119 tasks while retaining 1,400 across the 16 indexed rows; Humanoid Everyday to 8,949 episodes; DECO-50 to 8,000 trajectories; HapTile to 1,726 demonstrations; OmniVitac to the official lower-bound wording `21,000+ trajectories`; and DexMimicGen's title to the paper-reported `21K demos` while retaining 1,000 demos on each of the nine indexed main-task rows. Left MolmoB0T, RoboCasa365, and Genie Sim 3.0 unchanged because the available evidence does not establish an episode total for the data currently represented on the site. The latest main branch already replaced LET-Body with numeric LET-Base and LET-Dex sections. |
